...
String process(Mat inputimage, Rect *resultPoint);
...
Mat grayscale, image3, drawing; // Make images.
double maxratio = 0.9, minratio = 0.25, alpha = 0, beta = 500;

int main()
{
	Mat inputimage;
	cv::VideoCapture cap("C:\\Users\\이경신\\Desktop\\1.mp4");
	if (!cap.isOpened()) {
	...
	while (1)
	{
		...
		Rect resultRect;
		String busCarNumber = process(inputimage, &resultRect);

		if (busCarNumber == "error"){
			continue;
		}
		beforeRect = resultRect;
		beforeStr = busCarNumber;

		if (((abs(resultRect.br().x - resultRect.x / 2) <= (beforeRect.x + abs(beforeRect.br().x - beforeRect.x / 2)))) && ((abs(resultRect.br().x - resultRect.x / 2) >= (beforeRect.x - abs(beforeRect.br().x - beforeRect.x / 2))))){

			if (((abs(resultRect.br().y - resultRect.y / 2) <= (beforeRect.y + abs(beforeRect.br().y - beforeRect.y / 2)))) && (abs(resultRect.br().y - resultRect.y / 2) >= (beforeRect.y - abs(beforeRect.br().y - beforeRect.y / 2)))){
				if (busCarNumber == beforeStr){
					int count=0;
					count = count +1;
					if (count >= 30)
					{
