...
double maxratio = 0.9, minratio = 0.25, alpha = 0, beta = 500;
	...
	int main()
	{
		...
		while (1)
		{
			...
			int boundingX = 50;
			int boundingY = 50;
			double distBeforAfterFrameX = abs(((resultRect.br().x - resultRect.x) / 2) - abs((beforeRect.br().x - beforeRect.x) / 2)); //rectcenterx
			double distBeforAfterFrameY = abs(((resultRect.br().y - resultRect.y) / 2) - abs((beforeRect.br().y - beforeRect.y) / 2)); //rectcentery
